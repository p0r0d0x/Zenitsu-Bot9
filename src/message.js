...
async function GroupUpdate(DeepakBotInc, update) {
	try {
		for (let n of update) {
			...
			if (setinfo) {
				let profile;
				try {
					profile = await DeepakBotInc.profilePictureUrl(n.id, 'image');
				} catch {
					profile = 'https://telegra.ph/file/95670d63378f7f4210f03.png';
				}
				if (n.announce) {
					await DeepakBotInc.sendMessage(n.id, {
...
async function GroupParticipantsUpdate(DeepakBotInc, { id, participants, action }) {
	try {
		let welcome = global.db.groups[id].welcome
		if (welcome) {
			let metadata = await DeepakBotInc.groupMetadata(id);
			for (let n of participants) {
				let profile;
				try {
					profile = await DeepakBotInc.profilePictureUrl(n, 'image');
				} catch {
					profile = 'https://telegra.ph/file/95670d63378f7f4210f03.png';
				}
				DeepakWlcm = await getBuffer(profile)
                DeepakLft = await getBuffer(profile)
				if (action == 'add') {
				let deepakName = n
...
await DeepakBotInc.relayMessage(id, msgs.message, {})
				} else if (action == 'remove') {
				let deepakName = n
                const deepakdate = moment.tz('Asia/Kolkata').locale('en-IN').format('DD/MM/YYYY');
                const deepaktime = moment().tz('Asia/Kolkata').locale('en-IN').format('HH:mm:ss');
...
const deepakdate = moment.tz('Asia/Kolkata').locale('en-IN').format('DD/MM/YYYY');
let deepakName = n
deepakbody = ` 𝗖𝗼𝗻𝗴𝗿𝗮𝘁🎉 @${deepakName.split("@")[0]}, you have been *promoted* to *admin* 🥳`
   await DeepakBotInc.sendMessage(id,
 { text: deepakbody,
 contextInfo:{
 mentionedJid:[n],
...
async function MessagesUpsert(DeepakBotInc, message, store) {
	try {
		let botNumber = await DeepakBotInc.decodeJid(DeepakBotInc.user.id);
		const msg = message.messages[0];
		const type = msg.message ? (getContentType(msg.message) || Object.keys(msg.message)[0]) : '';
		if (!DeepakBotInc.public && !msg.key.fromMe && message.type === 'notify') return
		if (msg.key.id.startsWith('BAE5')) return
		if (msg.key.id.length === 22) return
		if (!msg.message) return
		const m = await Serialize(DeepakBotInc, msg, store)
		require('../Zenitsu9')(DeepakBotInc, m, message, store);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			...
			apb.key = msg.key
			apb.key.fromMe = areJidsSameUser(m.sender, DeepakBotInc.user.id);
			...
			DeepakBotInc.ev.emit('messages.upsert', pbr);
		}
		let antiswview = global.db.settings[botNumber].antiswview
		if (antiswview) {
			if (msg.key.remoteJid === 'status@broadcast') {
				await DeepakBotInc.readMessages([msg.key]);
				if (/protocolMessage/i.test(type)) DeepakBotInc.sendFromOwner(ownernumber, 'Status from @' + msg.key.participant.split('@')[0] + ' Was removed', msg, { mentions: [msg.key.participant] });
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					await DeepakBotInc.sendFromOwner(ownernumber, `Story from @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
				}
			}
		}
	} catch (e) {
...
async function Solving(DeepakBotInc, store) {
	DeepakBotInc.public = true
	
	DeepakBotInc.serializeM = (m) => MessagesUpsert(DeepakBotInc, m, store)
	...
	DeepakBotInc.getName = (jid, withoutContact  = false) => {
		const id = DeepakBotInc.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || DeepakBotInc.groupMetadata(id) || {};
...
return waMessage
	...
	DeepakBotInc.sendPoll = (jid, name = '', values = [], selectableCount = 1) => {
		return DeepakBotInc.sendMessage(jid, { poll: { name, values, selectableCount }})
	}
...
DeepakBotInc.sendImage = async (jid, path, caption = '', quoted = '', options) => {
	...
	DeepakBotInc.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		...
		if (mime.includes('gif')) {
			return DeepakBotInc.sendMessage(jid, { video: buffer.data, caption: caption, gifPlayback: true, ...options }, { quoted });
		} else if (mime === 'application/pdf') {
			return DeepakBotInc.sendMessage(jid, { document: buffer.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted });
		} else if (mime.includes('image')) {
			return DeepakBotInc.sendMessage(jid, { image: buffer.data, caption: caption, ...options }, { quoted });
		} else if (mime.includes('video')) {
			return DeepakBotInc.sendMessage(jid, { video: buffer.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted });
		} else if (mime.includes('audio')) {
			return DeepakBotInc.sendMessage(jid, { audio: buffer.data, mimetype: 'audio/mpeg', ...options }, { quoted });
		}
	}
  ...
  try {
	...
	DeepakBotInc.sendFakeLink = async (jid, text, title, body, thumbnail, myweb, options = {}) => {
		await DeepakBotInc.sendMessage(jid, {
			text: text,
			contextInfo: {
				externalAdReply: {
					title: title,
	...
	DeepakBotInc.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await DeepakBotInc.sendMessage(a + '@s.whatsapp.net', { text, ...options }, { quoted });
		}
	}
...
return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '@s.whatsapp.net')
	...
	DeepakBotInc.sendTextMentions = async (jid, text, quoted, options = {}) => DeepakBotInc.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	DeepakBotInc.sendAsSticker = async (jid, path, quoted, type, options = {}) => {
		let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0);
		if (options && (options.packname || options.author)) {
		...
		await DeepakBotInc.sendMessage(jid, { sticker: { url: buff }, ...options }, { quoted });
		return buff;
	}
	...
	DeepakBotInc.getFile = async (PATH, save) => {
		...
		filename = path.join(__filename, '../DeepakMedia/trash/' + new Date * 1 + '.' + type.ext)
	...
	DeepakBotInc.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await DeepakBotInc.getFile(path, true);
	 ...
	 DeepakBotInc.sendButtonImage = async (jid, buttons, quoted, opts = {}) => {
      var image = await prepareWAMessageMedia({
         image: {
            url: opts && opts.image ? opts.image : ''
         }
      }, {
         upload: DeepakBotInc.waUploadToServer
      })
      ...
      return DeepakBotInc.relayMessage(jid, message["message"], {
         messageId: message.key.id
      })
   }
   
   DeepakBotInc.sendButtonVideo = async (jid, buttons, quoted, opts = {}) => {
   ...
   DeepakBotInc.sendList = async (jid, sections, quoted, opts = {}) => {
      let message = generateWAMessageFromContent(jid, {
         viewOnceMessage: {
            message: {
               interactiveMessage: {
      ...
      DeepakBotInc.relayMessage(jid, message["message"], {
         messageId: message.key.id
      })
   }
	
	DeepakBotInc.sendListImage = async (jid, sections, quoted, opts = {}) => {
      var image = await prepareWAMessageMedia({
         image: {
            url: opts && opts.image ? opts.image : ''
         }
	...
	DeepakBotInc.sendButtonMsg = async (jid, body = '', footer = '', title = '', media, buttons = [], quoted, options = {}) => {
		...
		await DeepakBotInc.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
	}
	
	DeepakBotInc.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: DeepakBotInc.waUploadToServer });
			return imageMessage;
		}
...
async function Serialize(DeepakBotInc, m, store) {
	const botNumber = DeepakBotInc.decodeJid(DeepakBotInc.user.id)
	if (!m) return m
	if (m.key) {
		m.id = m.key.id
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, DeepakBotInc)
				return await Serialize(DeepakBotInc, q, store)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(DeepakBotInc.decodeJid(m.msg?.contextInfo?.participant), DeepakBotInc.decodeJid(DeepakBotInc?.user?.id)),
			...
			m.quoted.delete = () => {
				DeepakBotInc.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.copy = () => Serialize(DeepakBotInc, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
		...
		try {
			if (/^https?:\/\//.test(text)) {
				...
				if (/gif|image|video|audio|pdf/i.test(mime)) {
					return DeepakBotInc.sendFileUrl(chatId, text, caption, quoted, options)
				} else {
					return DeepakBotInc.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
				}
			} else {
				return DeepakBotInc.sendMessage(chatId, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
			}
